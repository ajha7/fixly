...
class NextDoorApiClient:
    """Client for interacting with the NextDoor API."""
    
    def __init__(self, api_key: Optional[str] = None, api_base_url: str = "https://api.nextdoor.com/v1"):
        """Initialize the NextDoor API client.
        
        Args:
            api_key: NextDoor API key (optional, will use env var if not provided)
            api_base_url: Base URL for NextDoor API
        """
        self.api_key = api_key or os.environ.get("NEXTDOOR_API_KEY")
        if not self.api_key:
            raise ValueError("NextDoor API key not provided and not found in environment variables")
    ...
    async def search_providers(self, category: str, location: Dict[str, Any], radius: float = 10.0, limit: int = 10) -> List[ProviderModel]:
        """Search for service providers using the NextDoor API.
        ...
        try:
            ...
            params = {
                "query": category,
                "location": self._format_location_for_api(location),
                "radius": radius,
                "limit": limit
            }
            ...
            # data = response.json()
            # providers = self._parse_api_response(data)
            
            # For now, we'll use mock data
            await self._simulate_delay()
            providers = self._generate_mock_api_providers(category, location, radius, limit)
            
            return providers
        except RequestException as e:
            logger.error(f"Request error searching NextDoor API: {str(e)}")
            return []
    ...
    async def get_provider_details(self, provider_id: str) -> Optional[ProviderModel]:
        ...
        try:
            ...
            # response.raise_for_status()
            # data = response.json()
            # provider = self._parse_api_provider_detail(data)
            ...
            logger.error(f"Request error getting provider details from NextDoor API: {str(e)}")
            return None
        except Exception as e:
    ...
    def _parse_api_response(self, data: Dict[str, Any]) -> List[ProviderModel]:
        """Parse API response data into provider models.
        ...
        # This would be the actual implementation for parsing the API response
        # For now, we'll return an empty list
        return providers
    
    def _parse_api_provider_detail(self, data: Dict[str, Any]) -> Optional[ProviderModel]:
        """Parse API provider detail data into a provider model.
        
        Args:
            data: API provider detail data
    ...
    def _generate_mock_api_providers(self, category: str, location: Dict[str, Any], radius: float, limit: int) -> List[ProviderModel]:
